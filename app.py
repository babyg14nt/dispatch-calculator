...
st.markdown("<p style='color: #64748B; font-size: 0.95rem; font-weight: 500; margin-top: 5px;'>Corporate Fleet Logistics, HOS Compliance & Financial Intelligence Center</p>", unsafe_allow_html=True)
st.markdown("<br>", unsafe_allow_html=True)

# --- Main Split Layout: Left (Inputs) vs Right (Results) ---
...
with left_column:
    ...
    with tc1:
        ...
        average_speed = st.number_input("Avg Speed (MPH)", min_value=10.0, value=55.0, step=1.0)
    with tc2:
        pickup_time = st.time_input("Pickup Time", time(8, 0))
        fixed_expenses = st.number_input("Other Expenses ($)", min_value=0.0, value=150.0, step=10.0)
    st.markdown("</div>", unsafe_allow_html=True)

with right_column:
    st.markdown("### 📊 Command Center Results")
    ...
    with m1:
        st.markdown(f"""
            <div class='metric-card'>
                ...
                <h3 style='color: {prof_color}; margin: 5px 0 0 0;'>${net_profit:,.2f}</h3>
            </div>
        """, unsafe_allow_html=True)
        st.markdown(f"""
            <div class='metric-card'>
    ...
    with m2:
        st.markdown(f"""
            <div class='metric-card'>
                ...
                <h3 style='color: {prof_color}; margin: 5px 0 0 0;'>${rpm:.2f}</h3>
            </div>
        """, unsafe_allow_html=True)
        st.markdown(f"""
            <div class='metric-card'>
